...
The MSI installs into %LocalAppData%\\PyTarkBuddy (see scripts/setup_msi.py), so a newer MSI
upgrades in place with no elevation and no UAC. boot_gate() runs before the main window: it asks
GitHub for the latest release, and if it is newer than ours it hands off to one detached updater
window that downloads the MSI (a determinate progress bar), waits for this process to exit (so the
running exe is never the one being overwritten), installs with msiexec /quiet (the bar switches to
a cycling marquee for that indeterminate stage), then closes and relaunches the app. One window
start to finish. The gate returns True when an update is under way, so the caller exits instead of
...
REPO = 'matthewmiglio/PyTarkBuddy'
...
# handles, and a DETACHED_PROCESS child inherits that and dies before running a line.
# CREATE_NO_WINDOW gives it its own hidden console, and CREATE_NEW_PROCESS_GROUP plus the DEVNULL
# handles below keep it clean and detached.
...
# Colors mirror gui/app.py so it reads as part of the app. Single-quoted paths: %LOCALAPPDATA%
...
$f.FormBorderStyle='None'; $f.StartPosition='CenterScreen'; $f.TopMost=$true; $f.Text='PyTarkBuddy'
$f.ClientSize=New-Object Drawing.Size(360,132); $f.BackColor=$body; $f.ShowInTaskbar=$true
$f.Icon=[Drawing.Icon]::ExtractAssociatedIcon('@EXE@')  # app icon in the taskbar on minimise

$tb=New-Object Windows.Forms.Panel; $tb.SetBounds(0,0,360,30); $tb.BackColor=$bar; $f.Controls.Add($tb)
$title=New-Object Windows.Forms.Label; $title.Text='PyTarkBuddy'; $title.ForeColor=$ink
...
def boot_gate():
    ...
    try:
        ...
        return False  # offline, rate-limited, bad json: just boot
    if not found:
        return False
    tag, url = found
    ...
    try:
        ...
        return False  # fall through to a normal boot


def demo():
    assert _asset({'assets': [{'name': 'notes.txt', 'browser_download_url': 'x'},
                              {'name': 'pytarkbuddy-v9-win64.msi', 'browser_download_url': 'ok'}]}) == 'ok'
